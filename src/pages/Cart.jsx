                ...
                {cart.map((item, index) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-sm text-darkwood/50 dark:text-warm-400 mt-0.5 capitalize">{item.category}</p>
                      ...
                      <div className="flex items-center gap-3 mt-3">
                        <div className="flex items-center bg-warm-100 dark:bg-dark-surface rounded-xl overflow-hidden">
                          ...
                          >
                            <Minus className="h-3.5 w-3.5 text-darkwood/60 dark:text-warm-300" />
                          </button>
                          ...
                          <button
                            ...
                            className="p-2 hover:bg-warm-200 dark:hover:bg-dark-border transition-colors"
                          >
                            <Plus className="h-3.5 w-3.5 text-darkwood/60 dark:text-warm-300" />
                          </button>
                        </div>
            ...
            <div className="lg:col-span-1">
              ...
              >
                ...
                <div className="flex gap-2">
                  ...
                  <button className="px-4 py-2.5 bg-warm-100 dark:bg-dark-surface text-darkwood/70 dark:text-warm-300 rounded-xl text-sm font-semibold hover:bg-warm-200 dark:hover:bg-dark-border transition-colors">
                    {t('cart.apply')}
                  </button>
                </div>
                ...
                <div className="space-y-3 pt-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-darkwood/50 dark:text-warm-400">{t('cart.subtotal')} ({cart.length} {t('cart.items')})</span>
                    <span className="font-semibold text-darkwood dark:text-warm-100">{formatPrice(subtotal)}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-darkwood/50 dark:text-warm-400">{t('cart.shipping')}</span>
                ...
                <div className="space-y-3 pt-2">
                  {[
                    ...
                    { icon: RotateCcw, text: t('cart.easyReturns'), color: 'text-darkwood dark:text-warm-300' },
                    ...
                    <div key={i} className="flex items-center gap-3 text-sm">
                      ...
                      <span className="text-darkwood/60 dark:text-warm-400">{item.text}</span>
                    </div>
                  ))}
                </div>

                <div className="pt-3 border-t border-warm-100 dark:border-dark-border">
                  <p className="text-xs text-warm-400 dark:text-warm-500 mb-2 text-center">{t('cart.weAccept')}</p>
                  <div className="flex items-center justify-center gap-3">
                    {['Visa', 'MC', 'PayPal', 'Apple'].map((method) => (
                      <div key={method} className="bg-warm-50 dark:bg-dark-surface rounded-lg px-3 py-1.5 text-xs font-bold text-darkwood/50 dark:text-warm-400">
                        {method}
                      </div>
                    ))}
                  </div>
